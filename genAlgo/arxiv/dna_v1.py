...
class DNA:
    ...
    def crossover(self, partner, midpt_bool):
        ...
        if not midpt_bool:
            ...
            # Prob of taking one of own genes in crossover
            if total_fitness < 1e-5: self_prob = 0.5
            else: self_prob = min( 0.8, max( 0.2, self.fitness / max(total_fitness, 1e-4) ) ) # [0.2, 0.8]
            ...
            midpt = min(max(3, np.random.randint(len(self.genes))), len(self.genes)-3) 
