...
def train_rf(X_tr, y_tr, X_te, n_iter = 35, num_folds = 5):
...
#             else: continue
        ...
        for idx in range(len(self.genes['models'])):
            ...
            if model is not None:
                test_preds, params = self.train_predict(model, X_tr, y_tr, X_te)            
    ...
    def train_predict(self, mod, X_tr, y_tr, X_te, num_folds = 5, n_iter = 10):
        ...
        if mod == 'rf': preds, params = train_rf(X_tr, y_tr, X_te)
            
        elif mod == 'xgb': preds, params = train_xgb(X_tr, y_tr, X_te)
...
#         el_net_ens_eval_preds = el_net.predict(pred_array)
    ...
    def get_genes(self, get_params = False):
        if get_params: 
            return {'Data':self.genes['data'], 'Preprocessing':self.genes['preproc'], 'Models':self.genes['models'], 'Params':self.genes['model_params']}
